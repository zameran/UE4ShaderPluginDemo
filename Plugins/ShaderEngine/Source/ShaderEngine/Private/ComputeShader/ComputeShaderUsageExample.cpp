...
// This has to be the same as in the compute shader's spec [X, X, 1]
...
FComputeShaderUsageExample::FComputeShaderUsageExample(float SimulationSpeed, int32 SizeX, int32 SizeY, ERHIFeatureLevel::Type ShaderFeatureLevel)
{
	...
	// I would have loved to go with the R8G8B8A8_UNORM approach, but unfortunately, it seems UE4 does not support this in an obvious way, 
	// which is why I chose the UINT format using packing instead :)
...
void FComputeShaderUsageExample::ExecuteComputeShader(float TotalElapsedTimeSeconds)
{
	...
	// This macro sends the function we declare inside to be run on the render thread. What we do is essentially just send this class and tell the render thread 
	// to run the internal render function as soon as it can.
	// I am still not 100% Certain on the thread safety of this, if you are getting crashes, 
	// depending on how advanced code you have in the start of the ExecutePixelShader function, you might have to use a lock :)
	ENQUEUE_UNIQUE_RENDER_COMMAND_ONEPARAMETER(FComputeShaderRunner, FComputeShaderUsageExample*, ComputeShader, this,
	{
